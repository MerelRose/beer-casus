...
<body>
    ...
    <?php 
        ...
        $sql = "SELECT `name`, `brewer`, `type`, `yeast`, `perc`, `purchase_price`, AVG(rating) FROM `beers` 
        INNER JOIN ratings on beers.id = ratings.beer_id
        GROUP BY `name`, `brewer`, `type`, `yeast`, `perc`, `purchase_price`
        order by beers.id;";
        $result = $conn->query($sql);
        ...
        // function dislikeBeer($conn, $beer_id, $id) {
        ...
        //     $sql_delete = "DELETE FROM likes WHERE beer_id = $beer_id AND unique_id = '$id'";
        ...
        if (isset($_SESSION['email'])) {
            ...
            echo "</div>";
            echo "<button class='btn' type='submit' name='rate' " . (hasRatedBeer($conn, $beer["id"], $unique_id) ? "disabled" : "") . ">Rate</button>";
            echo "</form>"; 
            echo "</div>";
            echo "</div>";
            echo "</div>";
